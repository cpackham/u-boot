...
extern int string_to_ip6(const char *s, struct in6_addr *addr);
...
extern int net_send_udp_packet6(uchar *ether, struct in6_addr *dest,
				int dport, int sport, int len);

/* handler for incoming IPv6 echo packet */
extern void net_ip6_handler(struct ethernet_hdr *et, struct ip6_hdr *ip6,
	int len);
