...
static void
ip6_send_ns(struct in6_addr *neigh_addr)
{
...
static void
ip6_send_na(uchar *eth_dst_addr, struct in6_addr *neigh_addr, struct in6_addr *target)
{
	struct nd_msg *msg;
	__u16 len;
	uchar *pkt;
	...
	case IPV6_NDISC_NEIGHBOUR_ADVERTISEMENT:
		...
		if ((memcmp(&ndisc->target, &net_nd_rep_packet_ip6,
			...
			memcpy(((struct ethernet_hdr *)net_nd_tx_packet)->et_dest,
					neigh_eth_addr, 6);
			net_send_packet(net_nd_tx_packet, net_nd_tx_packet_size);
